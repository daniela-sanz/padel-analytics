...
- guarda un resumen local de la sesion
...
- `data/session/FileSessionRepository.kt`
...
No es todavia una arquitectura por capas completa con `domain`, `Room` y navegacion formal.
...
3. se guarda con `FileSessionRepository`
...
sequenceDiagram
    ...
    participant R as FileSessionRepository
...
- `FileSessionRepository.kt`: guarda y lee sesiones desde un JSON local
...
La persistencia ahora es minima y consciente:

- no usa `Room` todavia
- guarda resumenes en un archivo JSON local
- el archivo se llama `session_summaries.json`
- se guarda en `context.filesDir`

Ventaja:

- simple
- suficiente para validar el flujo funcional

Limite:

- no guarda todas las muestras IMU
- no hay consultas complejas
- no es el estado final de arquitectura
...
## 16. En que punto exacto del MVP estais
...
## 17. Forma sencilla de contarlo en una defensa
...
5. `Estado actual`: la app ya registra sesiones simuladas y esta preparada para sustituir la fuente fake por BLE real.

## 18. Resumen visual final
...
## 19. Como usar esta pizarra en papel

Si vas a dibujarla a mano, te recomiendo hacer 4 zonas:
...
## 20. Frase corta para quedarte con la idea

La app actual no es todavia el producto final, pero si es una base funcional y explicable donde ya estan probados el arranque Android, la estructura visual, el transporte BLE simulado y el guardado local de sesiones.
